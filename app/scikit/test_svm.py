...
if __name__ == '__main__':
    ...
    """
    # test_gridsearchを参照
    est = SVC(C=1000, kernel='poly',gamma=0.0001,degree=2)    # パラメータ (C-SVC, RBF カーネル, C=1000)
    clf = OneVsRestClassifier(est)  #多クラス分類器One-against-restによる識別
    clf.fit(tr_vecs_rand, tr_labels_rand)
    pred = clf.predict(ts_vecs_rand)

    clf2 = SVC(C=1000, kernel='poly',gamma=0.0001,degree=2)    # パラメータ (C-SVC, RBF カーネル, C=1000)
